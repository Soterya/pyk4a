...
def load_sidecar(path: Path) -> Tuple[Dict, List[Dict]]:
    ...
    return payload.get("metadata", {}), payload.get("frames", [])
...
def format_lines(
    ...
    lines = [
        ...
        f"device_id={metadata.get('device_id', 'n/a')} serial={metadata.get('serial', 'n/a')}",
        f"mode={metadata.get('wired_sync_mode', 'n/a')} sub_delay_usec={metadata.get('subordinate_delay_off_master_usec', 'n/a')}",
    ...
    if sidecar_frame is not None:
        lines.extend(
            [
                f"host_before_ns={sidecar_frame.get('host_unix_before_get_ns', 'n/a')}",
                f"host_after_ns={sidecar_frame.get('host_unix_after_get_ns', 'n/a')}",
                f"host_mid_ns={sidecar_frame.get('host_unix_mid_get_ns', 'n/a')}",
            ]
        )
    return lines
...
def export_record(
    ...
    metadata, sidecar_frames = load_sidecar(sidecar_path)
    ...
    try:
        ...
        while True:
            ...
            if sidecar_frame is not None:
                timing_entry["host_mid_ns"] = sidecar_frame.get("host_unix_mid_get_ns")
            ...
            lines = format_lines(record_name, metadata, sidecar_frame, capture, frame_idx)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extract color and depth images from MKV recordings, overlay nanosecond timestamps, and plot pairwise timestamp differences."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("multi_mkv/local_master_subordinate"),
        help="Directory containing MKV files and matching .timestamps.json files.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("multi_mkv/local_master_subordinate_overlays"),
    ...
    if not record_sets:
        raise FileNotFoundError(f"No MKV + .timestamps.json pairs found in {args.input_dir}")
