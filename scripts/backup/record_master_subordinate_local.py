...
# Local single-PC setup: 2 devices total (1 MASTER + 1 SUBORDINATE)
...
OUT_DIR = Path("multi_mkv/local_master_subordinate")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Common base config for all devices, with differences applied in build_config() based on master vs subordinate role.
BASE_CONFIG = dict(
...
# Helper functions for building configs and managing sidecar JSON files with timestamps and metadata.
...
# Sidecar JSON management: we write a separate JSON file alongside each MKV recording, containing metadata and an array of frame timestamps and host timing info. This is useful for post-processing and analysis of synchronization and timing.
...
def main() -> None:
    # Check that expected number of devices are connected, and initialize PyK4A instances with appropriate configs for master vs subordinate roles.
    ...
    # Build list of cameras with their configs and PyK4A instances based on LOCAL_DEVICES specs.
    cameras = []
    ...
    # Start subordinate first, master last.
    ...
    # Start devices and print their status.
    ...
    # Set up recordings and sidecar JSON files for each camera.
    records = []
    for c in cameras:
    ...
    try:
        print("Recording locally (master + subordinate)... Press CTRL-C to stop.")
        while True:
            captures = []
            for r in records:
