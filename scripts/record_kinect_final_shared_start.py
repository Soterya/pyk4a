...
def wait_for_shared_start() -> None:
    start_ns_str = os.environ.get("RECORD_SHARED_START_EPOCH_NS")
    if not start_ns_str:
        return

    target_ns = int(start_ns_str)
    while True:
        now_ns = time.time_ns()
    ...
    start_delta_ms = (time.time_ns() - target_ns) / 1_000_000
    print(
        f"recording gate opened at host epoch ns={target_ns} "
        f"(start_delta_ms={start_delta_ms:.3f})"
    )


def main() -> None:
    out_dir, _ = make_recording_dir()
